...
namespace TkStar\LaunchPad\Web\Components\Database\PDO\QueryBuilder\Wheres {
	...
	if(!trait_exists('\TkStar\LaunchPad\Web\Components\Database\PDO\QueryBuilder\Wheres\OrWhereNotLike')){
		trait OrWhereNotLike {
			...
			public static function OrWhereNotLike(String $column = '', String $value = null, Callable $closure = null) : \TkStar\LaunchPad\Web\Database {
				self::Check_Activation();
				$database_object = self::Static();
				if(!Framework\Validator::IsNullOrEmpty($column) AND !Framework\Validator::IsNullOrEmpty($value)){
					self::$wheres[] = array('where_not_like' => $column, 'if' => $value, 'type' => 'or');
				}
	...
	return(\TkStar\LaunchPad\Web\Components\Database\PDO\QueryBuilder\Wheres\OrWhereNotLike::class);
}
?>